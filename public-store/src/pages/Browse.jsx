...
const Browse = () => {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-slate-950 px-6 py-12 transition-colors">
      {/* Decorative Glow */}
      <div className="absolute top-[20%] right-[-10%] w-[50%] h-[50%] bg-indigo-50 dark:bg-indigo-900/10 rounded-full pointer-events-none z-0 blur-3xl"></div>
      ...
      <div className="max-w-7xl mx-auto space-y-10 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  selectedCategory === cat.value
                    ...
                    : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-500/30'
                }`}
              >
                {cat.name}
              </button>
          ...
          <div className="relative w-full lg:max-w-xs order-1 lg:order-2">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 focus:border-indigo-500 dark:focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl py-3 pl-10 pr-4 text-xs text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none transition-colors shadow-sm"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
            ...
            <div className="col-span-full text-center py-24 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 shadow-sm">
              <Download className="w-12 h-12 text-slate-300 dark:text-slate-700 mx-auto mb-4" />
              <p className="text-slate-500 dark:text-slate-400 text-sm">No assets match your search terms.</p>
              <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Try selecting another category or typing another word.</p>
            </div>
          ) : (
            filteredTemplates.map((template) => (
              <div
                key={template._id}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 rounded-3xl overflow-hidden p-4 flex flex-col justify-between shadow-sm hover:shadow-md dark:hover:shadow-indigo-500/5 transition-all animate-fade-in"
              >
                <div>
                  {/* Thumbnail */}
                  <div className="relative group rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 aspect-[16/9] border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="absolute inset-0 bg-slate-900/5 dark:bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-300 pointer-events-none">
                      {/* Subtle overlay */}
                    </div>
                    {/* Category Overlay */}
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {template.tags?.slice(0, 3).map((tag, i) => (
                        <span
                          key={i}
                          className="text-[9px] bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded-full capitalize font-semibold"
                        >
                          {tag}
                        </span>
                      ))}
